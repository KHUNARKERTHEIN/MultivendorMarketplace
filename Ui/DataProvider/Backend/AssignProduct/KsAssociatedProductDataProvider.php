...
/**
 * Class KsAssociatedProductDataProvider
 * @package Ksolves\MultivendorMarketplace\Ui\DataProvider\Backend
 */
class KsAssociatedProductDataProvider extends \Ksolves\MultivendorMarketplace\Ui\DataProvider\Backend\Product\Listing\KsCatalogProductDataProvider
{
    /**
    ...
    */
    ...
    /**
     * KsAssociatedProductDataProvider constructor.
     * @param string $name
    ...
    public function __construct(
        ...
        if ($ksProductType == 'simple' || $ksProductType == 'virtual' || $ksProductType == 'downloadable') {
            $ksAssociatedProductIds = $this->ksProductHelper->getKsParentProductIds($ksProductId);
        } else {
            $ksChildProductIds = $this->ksProductHelper->getKsChildProductIds($ksProductId);
            $ksAssociatedProductIds = [];
            foreach ($ksChildProductIds as $ksChildProductId) {
                $ksAssociatedProducts = $this->ksProductHelper->getKsParentProductIds($ksChildProductId);

                $ksAssociatedProductIds = array_unique(array_merge($ksAssociatedProducts, $ksAssociatedProductIds));
            }
            $key = array_search($ksProductId, $ksAssociatedProductIds);
            if (false !== $key) {
                unset($ksAssociatedProductIds[$key]);
            }
        }

        // Filter according to seller id
        $this->collection->addFieldToFilter('entity_id', ['in' => $ksAssociatedProductIds]);
    }
}
