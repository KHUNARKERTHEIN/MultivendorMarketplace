...
/**
 * KsNewProductAlert.php Ui Class
 */
class KsNewProductAlert extends Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as &$ksitem) {
                if (isset($ksitem['ks_seller_new_product_alert'])) {
                    $ksName = $this->getData('name');
                    if ($ksitem['ks_seller_new_product_alert']) {
                        $ksitem[$ksName] = html_entity_decode('<label class="ks-switch">
                            <input type="checkbox" class="ks-checkbox ks-product" data-id='.$ksitem['id'].' checked>
                            <span class="ks-slider ks-round"></span>
                            </label>');
                    } else {
                        $ksitem[$ksName] = html_entity_decode('<label class="ks-switch">
                            <input class="ks-checkbox ks-product" data-id='.$ksitem['id'].' type="checkbox">
                            <span class="ks-slider ks-round"></span>
                            </label>');
                    }
                }
