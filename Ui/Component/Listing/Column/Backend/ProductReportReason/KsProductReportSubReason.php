...
namespace Ksolves\MultivendorMarketplace\Ui\Component\Listing\Column\Backend\ProductReportReason;
...
/**
 * KsProductReportSubReason Ui Class
 */
class KsProductReportSubReason extends Column
{
    const KS_URL_PATH_EDIT = 'multivendor/reportproduct/newsubreason';
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as &$ksitem) {
                if ($ksitem['ks_subreason']!=="") {
                    $ksName = $this->getData('name');
                    $ksitem[$ksName] = html_entity_decode("<a href='".$this->ksUrlBuilder->getUrl(self::KS_URL_PATH_EDIT, ['id' => $ksitem['id']])."' target='blank' title='".__('Edit Sub Reason')."'>".$ksitem['ks_subreason'].'</a>');
                }
            }
        }
        return $dataSource;
