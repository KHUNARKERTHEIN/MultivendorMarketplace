...
/**
 * KsStoreStatus Ui Class
 */
class KsStoreStatus extends Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as &$ksitem) {
                if (isset($ksitem['ks_store_status'])) {
                    $ksName = $this->getData('name');
                    if ($ksitem['ks_store_status']) {
                        $ksitem[$ksName] = html_entity_decode('<label class="ks-switch">
                            <input type="checkbox" class="ks-checkbox" data-id='.$ksitem['id'].' checked>
                            <span class="ks-slider ks-round"></span>
                            </label>');
                    } else {
                        $ksitem[$ksName] = html_entity_decode('<label class="ks-switch">
                            <input class="ks-checkbox" data-id='.$ksitem['id'].' type="checkbox">
                            <span class="ks-slider ks-round"></span>
                            </label>');
                    }
                }
