 ...
 */
class KsSaveInvoice extends \Magento\Backend\App\Action implements HttpPostActionInterface
{
    /**
     ...
     * @param KsSalesOrderItemFactory $ksItemFactory
     */
    public function __construct(
        ...
        KsSalesOrderItemFactory $ksItemFactory
    ) {
    ...
    protected function _prepareShipment($invoice)
    {
        ...
        foreach ($invoiceData['items'] as $ksItemId => $ksQty) {
            $ksOrderItem = $this->ksItemFactory->create()->getCollection()->addFieldToFilter('ks_sales_order_item_id', $ksItemId);
            if ($ksOrderItem->getSize() > 0 && $ksQty > $ksOrderItem->getFirstItem()->getKsQtyToShip()) {
                $invoiceData['items'][$ksItemId] = $ksOrderItem->getFirstItem()->getKsQtyToShip();
            }
        }
