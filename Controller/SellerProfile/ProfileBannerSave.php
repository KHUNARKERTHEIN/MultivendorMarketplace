...
/**
 * ProfileBannerSave  Controller class
 */
class ProfileBannerSave extends \Magento\Framework\App\Action\Action
{
    /**
     * @var UploaderFactor
     */
    ...
    public function execute()
    {
        ...
        if ($ksIsSeller == 1) {
            ...
            if (isset($ksBannerFiles['ks-profile-banner-upload']['name']) && $ksBannerFiles['ks-profile-banner-upload']['name'] != '') {
                try {
                    $ksUploaderFactories = $this->ksUploaderFactory->create(['fileId' => 'ks-profile-banner-upload']);
                    ...
                    if($ksData['ks-store-id'] == 0){
                            ...
                            foreach ($ksCollection as $key => $ksAllStoreviewData) {
                                ...
                                if ($ksModelCollection->getSize() != 0) {
                                    foreach ($ksModelCollection as $key => $ksStoreviewData) {
                                        ...
                                        if ($ksAllStoreviewData->getKsStoreBanner() == $ksStoreviewData->getKsStoreBanner()) {
                                            // set data in specific store view
                                            $ksStoreviewData->setKsStoreBanner($ksResult['file']);
                                        }
                                ...
                                $ksAllStoreviewData->setKsStoreBanner($ksResult['file']);
                        ...
                        if (count($ksSellerStoreCollection) != 0) {
                            ...
                            $ksCollection->setKsStoreBanner($ksResult['file']);
                            $ksCollection->save();
                        } else {
                            ...
                            $ksNewData['ks_store_banner'] = $ksResult['file'];
                ...
                if($ksData['ks-store-id'] != 0){
                    ...
                    if (count($ksSellerStoreCollection) != 0) {
                        ...
                        $ksCollection->setKsSellerId($ksData['ks-seller-id']);
                        $ksCollection->setKsStoreId($ksData['ks-store-id']);
                        $ksCollection->setKsStoreBanner($ksAllStoreCollection->getKsStoreBanner());
                        $ksCollection->save();
                    } else {
                        $ksAllStoreCollection = $this->ksSellerStoreFactory->create()->getCollection()->addFieldToFilter('ks_seller_id', $ksData['ks-seller-id'])->addFieldToFilter('ks_store_id', 0)->getFirstItem();
                        $ksNewData = [];
                        ...
                        $ksNewData['ks_seller_id'] = $ksData['ks-seller-id'];
                        $ksNewData['ks_store_id'] = $ksData['ks-store-id'];
                        $ksNewData['ks_store_banner'] = $ksAllStoreCollection->getKsStoreBanner();
