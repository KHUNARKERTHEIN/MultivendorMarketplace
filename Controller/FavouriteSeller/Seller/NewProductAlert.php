...
/**
 * Controller class for New Product Alert Status
 */
class NewProductAlert extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        if ($this->ksFavouriteSellerHelper->getKsCustomerId()) {
            ...
            if ($ksAction == "disable") {
                $ksNewProductAlertStatus = \Ksolves\MultivendorMarketplace\Model\KsFavouriteSeller::KS_STATUS_NEW_PRODUCT_ALERT_DISABLED;
            } else {
                $ksNewProductAlertStatus = \Ksolves\MultivendorMarketplace\Model\KsFavouriteSeller::KS_STATUS_NEW_PRODUCT_ALERT_ENABLED;
            }
            if ($ksId) {
                try {
                    $ksModel->setKsSellerNewProductAlert($ksNewProductAlertStatus);
                    $ksModel->save();

                    if ($ksAction == "disable") {
                        $ksMsg = "A follower's new product alert has been disabled successfully.";
                    } else {
                        $ksMsg = "A follower's new product alert has been enabled successfully.";
                    }
