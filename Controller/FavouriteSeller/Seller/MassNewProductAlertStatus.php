...
/**
 * MassNewProductAlertStatus Controller Class
 */
class MassNewProductAlertStatus extends \Magento\Framework\App\Action\Action
{
    /**
    ...
    /**
     * Check New Product Alert Status
     * Execute Action
     */
    public function execute()
    {
        ...
        if ($this->getRequest()->getParam('ks_seller_new_product_alert') == 1) {
            $ksStatus = 'enabled';
        } else {
            $ksStatus = 'disabled';
        }
        ...
        try {
            ...
            foreach ($ksCollection as $ksData) {
                if ($ksData->getKsSellerNewProductAlert() == 1 && $this->getRequest()->getParam('ks_seller_new_product_alert') == 1) {
                    $ksAlreadyEnabled++;
                } elseif ($ksData->getKsSellerNewProductAlert() == 0 && $this->getRequest()->getParam('ks_seller_new_product_alert') == 0) {
                    $ksAlreadyDisabled++;
                } else {
                    $ksData->setKsSellerNewProductAlert($this->getRequest()->getParam('ks_seller_new_product_alert'));
            ...
            if ($ksCount) {
                $this->messageManager->addSuccessMessage(
                    __('A total of %1 follower(s) new product alert has been %2.', $ksCount, $ksStatus)
                );
            }
            // error msg for already enabled
            if ($ksAlreadyEnabled) {
                $this->messageManager->addErrorMessage(
                    __('New product alert of already enabled followers can\'t be '.$ksStatus.'.')
                );
            }
            // error msg for already disabled
            if ($ksAlreadyDisabled) {
                $this->messageManager->addErrorMessage(
                    __('New product alert of already disabled followers can\'t be '.$ksStatus.'.')
                );
            }
        } catch (\Exception $e) {
