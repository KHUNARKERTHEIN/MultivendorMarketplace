...
/**
 * PriceChangeAlert Controller Class
 */
class PriceChangeAlert extends \Magento\Framework\App\Action\Action
{
    ...
    /**
     * Price change alert remove/save
     *
    ...
    public function execute()
    {
        if ($this->ksFavouriteSellerHelper->getKsCustomerId()) {
            try {
                ...
                if ($ksId) {
                    $ksModel->setKsCustomerPriceAlert($ksStatus);
                    $ksModel->save();
                    $this->ksSellerHelper->ksFlushCache();
                    $ksStoreName = $this->ksFavouriteSellerHelper->getKsStoreName($ksModel->getKsSellerId());
                    if ($ksAction == "remove") {
                        $this->ksMessageManager->addSuccess(__("\"%1\" store price change alert has been disabled successfully.", $ksStoreName));
                    } else {
                        $this->ksMessageManager->addSuccess(__("\"%1\" store price change alert has been enabled successfully.", $ksStoreName));
                    }
                }
            } catch (\Exception $e) {
