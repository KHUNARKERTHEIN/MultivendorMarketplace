 ...
 */
class KsSellerLocatorProductPageView extends \Magento\Config\Block\System\Config\Form\Field
{
    /**
    ...
    {
        return $this->ksRegistry->registry('ks_sellerLocator_assigned_seller_sortby');
    }
    ...
    {
        return $this->ksRegistry->registry('ks_sellerLocator_assigned_seller');
    }
    ...
    public function getKsProduct()
    {
        ...
        $ksCollection= $this->ksProductCollection->create()->addFieldToFilter('ks_product_id', $ksProduct->getId());
        return $ksCollection;
    }
    ...
    {
        $ksProduct = $this->ksRegistry->registry('current_product');
        return $ksProduct->getId();
    }
    
    /**
     * Get KsSeller Product Name
     * @return int
     */
    public function getKsSellerProductName($ksProductId)
    {
        $ksCollection= $this->ksCatalogCollection->create()->addFieldToFilter('entity_id', $ksProductId)->addAttributeToSelect('*');
        return $ksCollection->getFirstItem()->getName();
    }

    /**
     * Get KsProduct Seller Id
     * @return int
     */
    public function getKsSellerId()
    {
        $ksProduct = $this->ksRegistry->registry('current_product');
        $ksCollection= $this->ksProductCollection->create()->addFieldToFilter('ks_product_id', $ksProduct->getId())->getFirstItem()->getKsSellerId();
        return $ksCollection;
    }

    /**
     * Get Assigned Seller to products
     * @return array
     */
    public function getKsAssignedSeller()
    {
        $ksSellerCollection = $this->getKsSellerRegistry();
        if ($ksSellerCollection) {
            $ksSortBy = $this->ksRegistry->registry('ks_sellerLocator_assigned_seller_sortby');
            switch ($ksSortBy) {
                case "Name(AtoZ)":
