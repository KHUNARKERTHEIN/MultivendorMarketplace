...
namespace Ksolves\MultivendorMarketplace\Block\Adminhtml\Banners\Buttons;
...
class KsDeleteButton extends KsGenericButton implements ButtonProviderInterface
{
    ...
    {
        $ksId = $this->ksCoreRegistry->registry('current_banners_id');
        $ksData = [];
        if ($ksId) {
            $ksData = [
                'label' => __('Delete'),
    ...
    {
        return $this->getUrl('*/*/delete', ['id' => $this->ksCoreRegistry->registry('current_banners_id')]);
    }
}
