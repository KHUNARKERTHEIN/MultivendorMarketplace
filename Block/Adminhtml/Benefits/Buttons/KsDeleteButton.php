...
namespace Ksolves\MultivendorMarketplace\Block\Adminhtml\Benefits\Buttons;
...
class KsDeleteButton extends KsGenericButton implements ButtonProviderInterface
{
    ...
    {
        $ksId = $this->ksCoreRegistry->registry('current_benefits_id');
        $ksData = [];
        if ($ksId) {
            $ksData = [
                'label' => __('Delete'),
                ...
                'on_click' => 'deleteConfirm(\''
                    . __('Are you sure you want to delete this benefit ?')
                    . '\', \'' . $this->getDeleteUrl() . '\')',
                'sort_order' => 20,
            ];
        }
    ...
    {
        return $this->getUrl('*/*/delete', ['id' => $this->ksCoreRegistry->registry('current_benefits_id')]);
    }
}
