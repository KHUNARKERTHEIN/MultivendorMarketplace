...
define([
    ...
    $.widget('mage.ksUpsellDialog', {
         ...
         */
        _create: function () {
            this.$grid = this.element.find('[data-role=upsell-product-grid]');
            
            this.productTmpl = mageTemplate('#ks_upsell_product_template');
        ...
        _remove: function (event) { 
            var self= this;
            $('body').on('click', '[data-role=upsell-product-grid] [data-role=delete]', function(event){ 
         ...
         */
        _ksSort: function () { 
            if(this.$grid.find('[data-role=id]').length > 0){ 
                this.$grid.sortable({
                    distance: 8,
                    items: '[data-role=row]',
                    tolerance: 'pointer',
        ...
        _ksOpenModal: function () {  
            ...
            $(".ks-add-upsell-product").on('click',function(){ 
             
                var ksTitle = $(this).data('title');
                ko.cleanNode($("#ks_upsell_product_modal")[0]);
                ...
                $.ajax({
                    type: "POST",
                    url: url.build("multivendor/product_ajax/ksaddupsellproduct"),
                    showLoader: true,
                    data: [],
                    success: function (ksResponse) { 
                        uiRegistry.get(function(component){
                            if(component.name != undefined){
                                if (component.name.indexOf('ks_upsell_product_listing') != -1) {
                                    uiRegistry.remove(component.name);
                                }
                            }
                        });
                        $("#ks_upsell_product_modal").html(ksResponse);
                        $("#ks_upsell_product_modal").trigger('contentUpdated');
                        $("#ks_upsell_product_modal").applyBindings();
                      
                    }
                });

                modal(options, $("#ks_upsell_product_modal"));
                $("#ks_upsell_product_modal").modal("openModal");
            });
        },
        ...
        _ksAddSelectedProducts: function () { 
            ...
            $("#ks_upsell_product_modal .data-grid-checkbox-cell input[type='checkbox']:checked").each(function(){
