...
define([
    ...
    $.widget('mage.ksRelatedDialog', {
         ...
         */
        _create: function () { 
            this.$grid = this.element.find('[data-role=related-product-grid]'); 

            this.productTmpl = mageTemplate('#ks_related_product_template');
        ...
        _remove: function () { 
            var self= this;
            $('body').on('click', '[data-role=related-product-grid] [data-role=delete]', function(event){ 
        ...
        _ksOpenModal: function () {  
            ...
            $(".ks-add-related-product").on('click',function(){ 
                var ksTitle = $(this).data('title');
                ko.cleanNode($("#ks_related_product_modal")[0]);
                ...
                $.ajax({
                    type: "POST",
                    url: url.build("multivendor/product_ajax/ksaddrelatedproduct"),
                    showLoader: true,
                    data: [],
                    success: function (ksResponse) { 
                        uiRegistry.get(function(component){
                            if(component.name != undefined){
                                if (component.name.indexOf('ks_related_product_listing') != -1) {
                                    uiRegistry.remove(component.name);
                                }
                            }
                        });
                        $("#ks_related_product_modal").html(ksResponse);
                        $("#ks_related_product_modal").trigger('contentUpdated');
                        $("#ks_related_product_modal").applyBindings();
                    }
                });

                modal(options, $("#ks_related_product_modal"));
                $("#ks_related_product_modal").modal("openModal");
            });
        ...
        _ksAddSelectedProducts: function () { 
            ...
            $("#ks_related_product_modal .data-grid-checkbox-cell input[type='checkbox']:checked").each(function(){
