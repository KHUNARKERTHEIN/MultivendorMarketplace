...
define([
    ...
    'Ksolves_MultivendorMarketplace/js/product/gallery/ks-video-information'
], function ($) {
    'use strict';
    ...
    $.widget('mage.createVideoPlayer', {
        options: {
            videoId: '',
            videoProvider: '',
            container: '.ks-video-player-container',
            videoClass: 'product-video',
            reset: false,
            metaData: {
                DOM: {
                    title: '.ks-video-information.title span',
                    uploaded: '.ks-video-information.uploaded span',
                    uploader: '.ks-video-information.uploader span',
                    duration: '.ks-video-information.duration span',
                    all: '.ks-video-information span',
                    wrapper: '.ks-video-information'
                },
                data: {
                    title: '',
                    uploaded: '',
    ...
    $.widget('mage.updateInputFields', {
        options: {
            ...
            DOM: {
                ...
                fileField: '#ks_file_name',
                descriptionField: 'textarea[name="video_description"]',
                thumbnailLocation: '.ks_video_screenshot_preview .ks-field-control'
            },
            data: {
                url: '',
                title: '',
    ...
    $.widget('mage.ksNewVideoDialog', {
        ...
        _videoPreviewInputSelector: '#ks_video_screenshot',

        _videoPreviewRemoteSelector: '',

        _videoDisableinputSelector: '#new_video_disabled',

        _videoPreviewImagePointer: '#ks_video_screenshot_preview',

        _videoFormSelector: '#ks_new_video_form',

        _itemIdSelector: '#ks_item_id',

        _videoUrlSelector: '[name="video_url"]',

        _videoImageFilenameselector: '#ks_file_name',

        _videoUrlWidget: null,

        _videoInformationBtnSelector: '[name="ks_video_get"]',

        _editVideoBtnSelector: '#media_gallery_content .ks-form-image',
        ...
        _onImageLoaded: function (result, file, oldFile, callback) {
            ...
            if ($('#ks_video_url').parent().find('.image-upload-error').length > 0) {
                $('.image-upload-error').remove();
            }

            if (data.errorcode || data.error) {
                $('#ks_video_url').parent().append('<div class="image-upload-error">' +
        ...
        _create: function () {
            ...
            this.element.modal({
                type: 'slide',
                modalClass: 'mage-new-video-dialog',
        ...
        createVideoItemIcons: function () { 
            var $imageWidget = $(this._imageWidgetSelector).find('.ks-product-image.video-item'),
                $productGalleryWrapper = $(this._imageProductGalleryWrapperSelector).find('.ks-product-image.video-item');
        ...
        findElementId: function (file) {
            var elem = $('.ks-form-image.ks-item').find('input[value="' + file + '"]');
        ...
        saveImageRoles: function (imageData) {
            ...
            if (data && data.length > 0) {
                containers = $('.ks-image-placeholder').siblings('input');
        ...
        _onOpenDialog: function (e, imageData) {
            ...
            if (imageData['media_type'] === 'external-video') {
                ...
                file = $('#ks_file_name').val(imageData.file);

                $.each($('.ks_video_image_role'), function () {
                    $(this).prop('checked', false).prop('disabled', false);
                });

                $.each($('.ks-image-placeholder').siblings('input:hidden'), function () {
                    ...
                    if ($(this).val() === file.val()) {
                        ...
                        $('#ks_new_video_form input[value="' + imageRole + '"]').prop('checked', true);
                    }
                });
            }
        ...
        toggleButtons: function () {
            ...
            $('.video-placeholder, .ks-add-video-button-container > button').click(function () {
            ...
            /* $(document).on('click', '#media_gallery_content .ks-item.video-item', function () { 
                $(this).mage('ksOpenVideoModal');
            ...
            $(document).on('click', '#media_gallery_content .ks-item.video-item:not(.removed)', function () {
                var flagChecked,
                    file,
                ...
                file = $('#ks_file_name').val(container.find('input[name*="file"]').val());
