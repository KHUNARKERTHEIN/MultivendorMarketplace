...
define([
...
], function (Component, $, ksGridItem , ko, _, alert) {
    'use strict';
    ...
    return Component.extend({
        ...
        initialize: function () {
            this._super();
            var ksElement = $('#product-variations-matrix');
        ...
        _makeProduct: function (product) {
            var productId = product['entity_id'] || product.productId || null,
                ...
                options = _.map(attributes, function (option, attribute) {
                    ...
                    var ksAttribute = _.findWhere(this.attributes(),{code: attribute});
                    
                    if(!result.attribute_label){
                        result.attribute_label = ksAttribute.label;
                    }
        ...
        buildProductUrl: function (productId) { 
            return this.productUrl.replace('productid', productId);
        },
        ...
        removeProduct: function (rowIndex) {
         ...
         /**
         * change status product from grid
         * @param  rowIndex
         */
        ...
        initImageUpload: function () {
            require([
                    ...
                    $(this).fileupload({
                        ...
                        acceptFileTypes: /(\.|\/)(gif|jpe?g|png)$/i,
                            ...
                            if (!data.result.error) {
                                ...
                                parentElement.find('.ks-config-upload-div label').addClass('ks-config-file-upload-label');
                            } else {
                                alert({
                ...
                $('body').on('click', '[data-role=product-variations-matrix] [data-action=no-image]', function(e){ 
                    var parentElement = $(event.target).closest('[data-column=image]');
                    parentElement.find('.ks-image-container').remove();
                    parentElement.find('[name$="[image]"]').val('');
                    parentElement.find('.dropdown').remove();
                    //parentElement.find('[data-toggle=dropdown]').trigger('close.dropdown').hide();
                    parentElement.find('.ks-config-upload-div label').removeClass('ks-config-file-upload-label');
                });
            });
        },
         ...
         * @returns {*}
         */
        renderedPager: function () {
            var ksElement = $('#product-variations-matrix');

            ksGridItem._ksUpdatePager(ksElement);
            ksGridItem._ksPagingNumerUpdate(ksElement);
            ksGridItem._ksChangePager(ksElement);
        },
